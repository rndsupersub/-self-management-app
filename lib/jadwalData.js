...
export const JADWAL_KERJA = [
  ...
  { id: "npd", waktuMulai: "09:00", waktuSelesai: "10:00", label: "📚 NPD", unit: "10 halaman" },
...
// ========== TEMPLATE DEFAULT (UNTUK USER BARU) ==========
...
// ========== HELPER: AMBIL JADWAL UNTUK TANGGAL TERTENTU ==========
export function getJadwalUntukTanggal(jadwalUser, dateStr) {
  const date = new Date(dateStr);
  const day = date.getDay(); // 0 = Minggu, 1 = Senin, ..., 6 = Sabtu

  // Ambil jadwal dari Firestore, fallback ke default
  const jadwalSumber = jadwalUser?.kerja && jadwalUser?.minggu
    ? jadwalUser
    : DEFAULT_JADWAL;
  ...
  // Filter untuk hari tertentu (Fikih dll)
