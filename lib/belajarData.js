...
// Warna bar di kalender: default per kategori, user bisa ubah per tool.
    ...
    subKategori: [
      {
        ...
        nama: "Fiqih Syafii",
        tools: [
          { id: "bab_thaharah", nama: "Bab Thaharah", warna: "green", fitur: [] },
          { id: "bab_shalat", nama: "Bab Shalat", warna: "green", fitur: [] },
        ],
      },
      {
        id: "tauhid",
...
// Path: ["design", "2d", "illustrator"] → return warna dari tool illustrator.
// Kalau nggak ketemu, fallback ke warna kategori.
export function getWarnaToolByPath(kategoriData, path) {
  ...
  // Kalau item punya warna, pakai itu
  if (result.item.warna) return result.item.warna;
  // Kalau nggak, cari tool-nya (naik satu level)
  if (path.length >= 3) {
...
// Append catatan log ke fitur materi. Return kategori yang udah di-update.
export function syncLogToMateri(kategoriData, log) {
  ...
  // Kalau log punya catatan, append
...
// Bikin log entry dari catatan yang diisi di halaman materi.
// Return { kategori, logHarian } yang udah di-update.
export function syncMateriToLog(kategoriData, logHarian, path, catatan, gdriveUrl, tanggal) {
  ...
  const logEntry = {
    ...
    gdriveUrl,
    telegramMessageId: "",
    sumber: "materi", // biar bisa dibedakan dari log kalender
...
// Hapus log, dan hapus juga dari materi (kalau log-nya sumbernya dari materi).
export function hapusLogSync(kategoriData, logHarian, log, tanggal) {
  ...
  // Kalau log sumbernya dari materi, hapus juga dari materi
...
export function updateLogSync(kategoriData, logHarian, oldLog, newLog, tanggal) {
  ...
  // Update materi (replace catatan lama dengan yang baru)
  let newKategori = kategoriData;
