        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="text-sm breadcrumbs mb-6">
            <ul>
              ...
              <li><a onClick={() => router.push("/dashboard/youtube")} className="cursor-pointer">📺 YouTube</a></li>
          ...
          <div className="tabs tabs-boxed bg-white shadow border border-gray-200 mb-4 p-1 w-fit">
            <button className={`tab ${activeTab === "kalender" ? "tab-active bg-blue-600 text-white" : ""}`} onClick={() => setActiveTab("kalender")}>📅 Kalender</button>
            <button className={`tab ${activeTab === "daftar" ? "tab-active bg-blue-600 text-white" : ""}`} onClick={() => setActiveTab("daftar")}>📋 Daftar Konten</button>
          </div>
          ...
          {activeTab === "daftar" && (
            <div className="card bg-white shadow border border-gray-200">
              <div className="card-body p-4">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-lg font-bold text-gray-800">📋 Semua Konten</h2>
                  <button className="btn btn-primary btn-sm" onClick={() => openModal()}>+ Tambah Konten</button>
                </div>
                {channelLogs.length === 0 ? (
                  <p className="text-center text-gray-400 py-4">Belum ada konten di channel ini.</p>
                ) : (
                  <div className="space-y-2">
                    {channelLogs.sort((a,b) => new Date(b.tanggal) - new Date(a.tanggal)).map(log => {
                      const statusObj = KONTEN_STATUS.find(s => s.id === log.status);
                      return (
                        <div key={log.id} className="bg-gray-50 border border-gray-200 p-3 rounded-lg flex justify-between items-center cursor-pointer hover:border-blue-400" onClick={() => openModal(log.tanggal, log)}>
                          <div>
                            <p className="font-bold text-sm text-gray-800">{log.judul}</p>
                            <p className="text-xs text-gray-500">📅 {log.tanggal} • {log.tipe === "short" ? "📱 Short" : "🖥️ Video"}</p>
                          </div>
                          <span className={`text-xs px-2 py-1 rounded text-white ${statusObj?.color}`}>{statusObj?.nama}</span>
                        </div>
                      );
                    })}
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white w-11/12 max-w-lg rounded-xl shadow-xl overflow-hidden flex flex-col">
            <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
              <h3 className="font-bold text-lg text-gray-800">{form.id ? "✏️ Edit Konten" : `➕ Tambah Konten`}</h3>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-500 font-bold">✕</button>
            </div>
            <form id="youtubeForm" onSubmit={handleSave} className="p-4 space-y-4 overflow-y-auto max-h-[70vh]">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Judul Konten</label>
                <input type="text" className="input input-bordered w-full bg-white text-gray-800" value={form.judul} onChange={e => setForm({...form, judul: e.target.value})} required />
              </div>
            ...
            <div className="p-4 border-t border-gray-200 bg-gray-50 flex justify-between">
              {form.id ? <button type="button" onClick={() => handleDelete(form.id)} className="btn btn-outline btn-error btn-sm">🗑️ Hapus</button> : <div></div>}
              <div className="space-x-2">
                <button type="button" onClick={() => setIsModalOpen(false)} className="btn btn-ghost btn-sm text-gray-600">Batal</button>
