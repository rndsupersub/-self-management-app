...
export default function BelajarDetailPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (docSnap.exists()) {
        ...
        await setDoc(docRef, { activities: DEFAULT_ACTIVITIES, belajar: { kategori: DEFAULT_KATEGORI }, belajarLogHarian: {}, belajarTargetHarian: DEFAULT_TARGET_HARIAN });
  ...
  useEffect(() => {
    ...
    if (isLeaf4 || isLeaf5Plus) setActiveTab("hasil-belajar");
  ...
  const handleRename = async (itemId, currentNama, e) => {
    ...
    const newPath = [...path, itemId];
    const newKategori = updateItem(kategori, newPath, { nama: newNama.trim() });
    await simpanKategori(newKategori);
  };
  ...
  if (!item) {
    return (
      <div className="h-screen flex flex-col bg-base-200">
        <div className="navbar bg-base-100 shadow px-4"><div className="flex-1"><h1 className="text-xl font-bold">🌙 Self Management</h1></div></div>
        <div className="flex-1 flex items-center justify-center">
          <div className="card bg-white shadow border border-gray-200">
            <div className="card-body p-8 text-center text-gray-400">
              ...
              <button className="btn btn-primary btn-sm" onClick={() => router.push("/dashboard/belajar")}>← Balik ke Belajar</button>
            </div>
          </div>
        </div>
      </div>
    ...
    return (
      <div className="text-sm breadcrumbs mb-6">
        <ul>
          {items.map((b, idx) => (
            <li key={idx}>
              {idx < items.length - 1 ? (<a onClick={() => router.push(b.path)} className="cursor-pointer">{b.nama}</a>) : (<span>{b.nama}</span>)}
            </li>
          ))}
        </ul>
      </div>
              ...
              <div className="card-body p-4">
                <div className="flex justify-between items-start gap-2">
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button className="btn btn-ghost btn-xs text-blue-500" onClick={(e) => handleRename(sub.id, sub.nama, e)} title="Rename">✏️</button>
  ...
  const renderTabs = (tabs) => (
    <div className="tabs tabs-boxed bg-white shadow border border-gray-200 mb-4 p-1 w-fit">
      {tabs.map((t) => (
        <button key={t.id} className={`tab ${activeTab === t.id ? "tab-active bg-blue-600 text-white" : ""}`} onClick={() => setActiveTab(t.id)}>{t.label}</button>
      ))}
    </div>
  );
  ...
  const renderContent = () => {
    if (level === 1) {
      return (
        <>
          {renderTabs([{ id: "materi", label: "📚 Materi" }, { id: "kalender", label: "📅 Kalender" }])}
          {activeTab === "materi" && (
            <>
              ...
              {showColorPicker && (
                <div className="card bg-white shadow border border-blue-300 mb-4">
                  <div className="card-body p-4">
                    ...
                    <div className="flex justify-end mt-3"><button className="btn btn-ghost btn-xs text-gray-600" onClick={() => setShowColorPicker(false)}>Tutup</button></div>
                  </div>
                </div>
              )}
              {renderList(item.fitur || [])}
              {punyaKaryaMingguan && (
                <div className="mt-6"><KaryaMingguan karyaMingguan={item.karyaMingguan || []} onUpdate={(newKarya) => { handleUpdateItem({ karyaMingguan: newKarya }); }} /></div>
              )}
            </>
          )}
          {activeTab === "kalender" && (
    ...
    if (level >= 4) {
      ...
      return (
        <>
          <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
            <p className="text-xs text-gray-500 italic">📂 Ini halaman leaf. Mau tambah sub-fitur di sini? Klik tombol →</p>
            <button className="btn btn-outline btn-xs" onClick={() => setShowForm(!showForm)}>+ Tambah Fitur</button>
          </div>
          {renderTabs([{ id: "hasil-belajar", label: "📝 Hasil Belajar" }, { id: "kalender", label: "📅 Kalender" }])}
          {activeTab === "hasil-belajar" && (
            <BelajarUpload item={item} onUpdate={handleUpdateItem} path={path} kategoriId={kategoriUtama} kategoriData={kategori} logHarian={logHarian} onUpdateLog={handleUpdateLog} today={today} mode="hasil-belajar" />
          )}
          {activeTab === "kalender" && (
            <KalenderBelajar kategoriId={kategoriUtama} subKategoriId={subKategoriUtama} kategoriData={kategori} logHarian={logHarian} targetHarian={targetHarian} onUpdateLog={handleUpdateLog} onUpdateTarget={handleUpdateTarget} subKategoriSiblings={subKategoriSiblings} onPindahSubKategori={handlePindahSubKategori} />
          )}
